...
public class WalletProtobufSerializer {
    ...
    private static Protos.Transaction makeTxProto(WalletTransaction wtx) {
        ...
        for (TransactionOutput output : tx.getOutputs()) {
            ...
            byte[] vReserve = output.getReserve();
            if (vReserve == null) {
                outputBuilder = Protos.TransactionOutput.newBuilder()
                ...
                outputBuilder = Protos.TransactionOutput.newBuilder()
                        .setScriptBytes(ByteString.copyFrom(output.getScriptBytes()))
                        .setValue(output.getValue().value)
                        .setUnlockHeight(output.getLockHeight())
                        .setVReserve(ByteString.copyFrom(output.getReserve()));
            }
    ...
    private static void writeConfidence(Protos.Transaction.Builder txBuilder,
        ...
        synchronized (confidence) {
            ...
            switch (ixType) {
                case IX_LOCKED:
                    confidenceBuilder.setIxType(Protos.TransactionConfidence.IXType.IX_LOCKED);
                    break;
                case IX_REQUEST:
                    confidenceBuilder.setIxType(Protos.TransactionConfidence.IXType.IX_REQUEST);
                    break;
                case IX_NONE:
                default:
                    confidenceBuilder.setIxType(Protos.TransactionConfidence.IXType.IX_NONE);
                    break;
            }
        }
     ...
     * to replay transactions from the blockchain (as if the wallet had been loaded and {@link Wallet}
    ...
    public Wallet readWallet(NetworkParameters params, @Nullable WalletExtension[] extensions,
        ...
        if (forceReset) {
            ...
            for (Protos.Transaction txProto : walletProto.getTransactionList()) {
                WalletTransaction wtx = connectTransactionOutputs(wallet.getParams(), txProto);
    ...
    private void readTransaction(Protos.Transaction txProto, NetworkParameters params) throws UnreadableWalletException {
        ...
        for (Protos.TransactionOutput outputProto : txProto.getTransactionOutputList()) {
            ...
            byte[] vReserve = outputProto.getVReserve().toByteArray();
            if (vReserve == null) {
                output = new TransactionOutput(params, tx, value, scriptBytes);
            } else {
                long lockHeight = outputProto.getUnlockHeight();
                output = new TransactionOutput(params, tx, value, scriptBytes, lockHeight, vReserve);
            }
            output.setTxVersion(txProto.getVersion());
            tx.addOutput(output);
        }
        ...
        if (tx.getVersion() >= SafeConstant.APP_TX_VERSION) {
...
//            log.info("---------output = {}", output.getAddressFromP2PKHScript(Constants.NETWORK_PARAMETERS).toBase58());
            if (transactionOutput.hasSpentByTransactionHash()) {
                final ByteString spentByTransactionHash = transactionOutput.getSpentByTransactionHash();
//                log.info("---------output = {}, {}", byteStringToHash(spentByTransactionHash), transactionOutput.getSpentByTransactionIndex());
    ...
    private void readConfidence(final NetworkParameters params, final Transaction tx,
        ...
        switch (confidenceProto.getIxType()) {
            ...
            case IX_NONE:
            default:
                confidence.setIXType(TransactionConfidence.IXType.IX_NONE);
                break;
