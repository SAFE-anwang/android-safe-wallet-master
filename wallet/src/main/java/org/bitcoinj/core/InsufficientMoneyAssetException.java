 ...
 */
public class InsufficientMoneyAssetException extends Exception {
    /** Contains the number of satoshis that would have been required to complete the operation. */
    @Nullable
    public final Coin missing;


    protected InsufficientMoneyAssetException() {
        this.missing = null;
    }

    public InsufficientMoneyAssetException(Coin missing) {
        this(missing, "Insufficient money,  missing " + missing.toFriendlyString());
    }

    public InsufficientMoneyAssetException(Coin missing, String message) {
        super(message);
