...
public class BitcoinURI {
    ...
    public BitcoinURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        if (input.startsWith(blockchainInfoScheme)) {
            ...
            if (colonIndex > 0) {
                ...
                if (coinURIScheme.equalsIgnoreCase(CoinDefinition.coinURIScheme)) {
                    schemeSpecificPart = input.substring(coinURIScheme.length() + 1);
                } else {
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (sepIndex == 0)
                throw new BitcoinURIParseException("Malformed Bitcoin URI - empty name '" +
                        nameValuePairToken + "'");
            final String nameToken = nameValuePairToken.substring(0, sepIndex);
            final String valueToken = nameValuePairToken.substring(sepIndex + 1);

            // Parse the amount.
            if (FIELD_AMOUNT.equals(nameToken)) {
                // Decode the amount (contains an optional decimal component to 8dp).
                putWithValidation(FIELD_AMOUNT, valueToken);
            } else {
    ...
    public String getAssetName() {
        return (String) parameterMap.get(FIELD_ASSET_NAME);
    }
    ...
    public String getAmount() {
        return (String) parameterMap.get(FIELD_AMOUNT);
    }
     ...
     */
    public static String convertToBitcoinURI(Address address, String amount,
                                             String label, String message, String assetName) {
        return convertToBitcoinURI2(address.getParameters(), address.toString(), amount, label, message, assetName);
    }
     ...
     * @return A String containing the Bitcoin URI
     */
    public static String convertToBitcoinURI2(NetworkParameters params,
                                              String address,
        ...
        if (assetName != null && !"".equals(assetName)) {
            ...
            builder.append(FIELD_ASSET_NAME).append("=").append(encodeURLString(assetName));
        }
