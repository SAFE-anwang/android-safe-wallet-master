...
//                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    @Nullable
    public static Address getToAddressOfSent(final Transaction tx, final Wallet wallet, String assetId) {
        for (final TransactionOutput output : tx.getOutputs()) {
            try {
                if (!output.isMine(wallet)) {
                    if (assetId.equals(SafeConstant.SAFE_FLAG) && output.isSafeTx()) { //safe资产交易
                        final Script script = output.getScriptPubKey();
                        return script.getToAddress(Constants.NETWORK_PARAMETERS, true);
                    } else if (output.isCurrentAssetTx(assetId)) { //其他资产交易
                        final Script script = output.getScriptPubKey();
                        return script.getToAddress(Constants.NETWORK_PARAMETERS, true);
                    }
                }
    ...
    @Nullable
    public static Address getWalletAddressOfReceived(final Transaction tx, final Wallet wallet, String assetId) {
        for (final TransactionOutput output : tx.getOutputs()) {
            try {
                if (output.isMine(wallet)) {
                    if (assetId.equals(SafeConstant.SAFE_FLAG) && output.isSafeTx()) { //safe资产交易
