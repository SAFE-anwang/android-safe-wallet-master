...
public final class BitcoinIntegration {
    ...
    private static final String MIMETYPE_PAYMENTREQUEST = "application/safe-paymentrequest"; // BIP 71
     ...
     * intent can be received by overriding {@link Activity#onActivityResult(int, int, Intent)}. Result indicates
     ...
     * @param requestCode
     *            Code identifying the call when {@link Activity#onActivityResult(int, int, Intent)} is called
     *            back
     * @param address
     *            Bitcoin address
     */
     ...
     * received by overriding {@link Activity#onActivityResult(int, int, Intent)}. Result indicates either
     ...
     * {@link Activity#onActivityResult(int, int, Intent)}. Result indicates either {@link Activity#RESULT_OK} or
    ...
    private static Intent makeBitcoinUriIntent(final String address, final Long amount) {
        final StringBuilder uri = new StringBuilder("safe:");
        if (address != null)
            uri.append(address);
        if (amount != null)
            uri.append("?amount=")
